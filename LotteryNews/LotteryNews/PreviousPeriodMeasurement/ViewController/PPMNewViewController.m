...
@interface PPMNewViewController ()<LNScrollerViewDelegate,UIScrollViewDelegate,UITableViewDelegate,UITableViewDataSource,PPMNewTableViewCellDelegate,TriolionFootCellDelagate>
...
- (void)viewDidLoad {
    ...
    self.navigationItemTitle = @"排列三预测大师";
    [self rankList];
...
- (void)setBasice{
    ...
    _ppmTableView.tableHeaderView = _topAdScrollerView;
...
- (void)rankList{
    ...
    [[UserStore sharedInstance]expert_rank:dict sucess:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [weakself.ppmTableView reloadData];
            [self startTimer];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        cell.delegate = self;
        LottoryCategoryModel *categoryModel = [[LNLotteryCategories sharedInstance].categoryArray objectAtIndex:indexPath.row];
        if (_rankListArray.count > indexPath.row) {
            RankListModel *rankModel = [_rankListArray objectAtIndex:indexPath.row];
            [cell setContent:categoryModel RankListModel:rankModel];
        }
